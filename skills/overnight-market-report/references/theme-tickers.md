...
한국 투자자의 실전 해석에 필요한 커버리지를 3개 tier로 나눕니다. 고정 30~50개를 매일 조회하는 대신, 뉴스 키워드·달력 이벤트에 따라 동적으로 확장합니다.

## 🥇 Tier 1 — Always-on (매일 고정)

메인 orchestrator의 1차 병렬 배치에서 **무조건 수집**하는 티커. 한국장 해석의 최소 공통분모입니다.
...
### 한국 반도체 밸류체인 (3개, v0.3.0 추가)
| 티커 | 해석 포인트 |
|---|---|
| **SMH** | 반도체 ETF — 삼성·SK하이닉스 방향성 프록시 (KOSPI 반도체주와 동조율 ~95%) |
| **SOXX** | iShares 반도체 ETF — SMH 교차검증 |
| **MU** | 마이크론 — HBM 경쟁사, SK하이닉스 직접 비교 대상 |

### 금융 메가캡 (2개, v0.3.0 추가)
| 티커 | 해석 포인트 |
|---|---|
| **JPM** | 실적·금리 해석의 바로미터, 은행 크레딧 스트레스 조기 탐지 |
| **BAC** | JPM 교차검증 — NIM 트렌드 |

### 에너지·헬스케어 개별 (3개, v0.3.0 추가)
| 티커 | 해석 포인트 |
|---|---|
| **XOM** | 엑슨모빌 — XLE 내부 리더십, 지정학 뉴스 반영 속도 |
| **LLY** | 일라이 릴리 — XLV 지배 종목(체중감량·당뇨 테마) |
| **UNH** | 유나이티드헬스 — 의료보험 정책 이슈 반영 |

### 중국 익스포저 (3개, v0.3.0 추가)
| 티커 | 해석 포인트 |
|---|---|
| **FXI** | 중국 본토 대형주 ETF — 관세·정책 충격 1차 반응 |
| **KWEB** | 중국 인터넷 ETF — 규제·소비 센티먼트 |
| **BABA** | 알리바바 개별 — 중국 개별주 모멘텀 대표 |

### 리스크 심리·금리 (2개, v0.3.0 추가)
| 티커 | 해석 포인트 |
|---|---|
| **HYG** | 하이일드 채권 ETF — 약세 = 크레딧 스프레드 확대(risk-off) |
| **TLT** | 장기 국채 ETF — 강세 = 금리 하락·안전자산 선호 |
...
**Tier 1 합계**: ETF/주식 ≈ **43개** + 환율 2건. 한 메시지에서 finnhub 43 + alphavantage 2 병렬 호출하면 전체 1차 수집 완료 (finnhub 무료 60 calls/min 한도 내).
...
| "tariff", "china trade", "trump tariff", "export control" | **FXI, KWEB, CPER, MCHI** | 중국·구리 — KOSPI 수출주 파급 |
...
| "semiconductor earnings", "wafer", "lithography", "asml" | **AMAT, LRCX, KLAC, ASML** | 반도체 장비 — 삼성·하이닉스 capex |
...
| **중국 지표 발표일** | news-harvester 결과에 "China PMI", "China CPI", "China GDP" 포함 | `FXI` 심화 + `CPER` (구리는 이미 Tier 2에 있을 수 있음 — 중복 제거) |
...
- **하드 캡**: 1 + 2 + 3 합쳐 **finnhub 호출 55개 이하** (분당 60 한도 여유)
- **중복 제거**: Tier 2/3에서 이미 Tier 1에 있는 티커는 스킵 (예: TSLA, USO, XOM)
