...
- **3-Tier 티커 유니버스 (v0.3.0)** — 매일 고정 **Tier 1(~43개)** + 뉴스 키워드 기반 **Tier 2(테마 트리거)** + 달력 이벤트 기반 **Tier 3** 구조. 한국 반도체 밸류체인(SMH/SOXX/MU), 금융 메가캡(JPM/BAC), 에너지·헬스케어 개별(XOM/LLY/UNH), 중국 익스포저(FXI/KWEB/BABA), 리스크 심리(HYG/TLT)까지 커버
- **Preflight 24h 캐싱 (v0.3.0)** — 이미 세팅된 사용자는 매 실행 10~20초 지연 없이 바로 리포트 생성
- **하이브리드 오케스트레이션** — 메인 세션이 `finnhub get_quote`로 시세를 병렬 수집하고, 별도 `news-harvester` 서브에이전트가 `Tavily search/extract + WebSearch`로 뉴스·매크로 이벤트를 격리 수집합니다. 메인 컨텍스트가 대용량 뉴스 원문으로 오염되지 않음
- **인과 분석 + 출처 링크 의무화 (v0.3.0)** — 단순 숫자 나열이 아니라 "왜 그렇게 움직였는지"를 섹터 로테이션·원자재·매크로 이벤트와 연결해 설명. 본문의 모든 인과 주장에 inline `[[n]](url)` 출처 링크 부착
- **섹터 로테이션 자동 라벨 (v0.3.0)** — risk-on / risk-off / 리플레이션 / 스태그플레이션 우려 / 혼조 중 하나를 섹터 섹션 최상단에 자동 표기
- **전일 대비 diff (v0.3.0)** — 직전 보고서를 자동으로 읽어 TL;DR·지수·로테이션 라벨 변화를 한눈에
- **YAML frontmatter 메타 (v0.3.0)** — `date_kst / session_date / tl_dr / indices / rotation_label`을 구조화해 저장, 주간 요약·트렌드 대시보드 파싱 용이
- **두 버전 자동 생성** —
  1. **원본 리포트** (전문가용, 디버전스·섹터 로테이션·FedWatch 같은 전문용어 사용)
  2. **쉬운버전** (입문자용, 상단 미니 용어집 + 본문에 괄호 해설 + 표 아래 "쉬운 해석" 삽입)
- **US 휴장/주말 자동 가드 (v0.3.0)** — 실행 시점의 한국 요일과 US 공휴일을 반영해 기준 세션 날짜를 결정
- **Sanity check 게이트 (v0.3.0)** — 지수 전면 N/A, 변동률 ±10% 초과, 환율 전면 실패 등 이상치 감지 시 리포트 상단 ⚠️ 배너
...
0. **YAML frontmatter** (date_kst / session_date / generated_at / tl_dr / indices / rotation_label)
1. 핵심 요약 (TL;DR)
2. 미국 3대 지수 (SPY/QQQ/DIA/IWM + VIX 프록시)
3. 섹터 동향 (XL* ETF 11종 일간 변동률 정렬 + **로테이션 라벨**)
4. 주요 종목 (Mag7 + AI 파급 + 반도체·금융·에너지·헬스케어 개별)
5. **테마별 트리거 종목** (Tier 2/3 활성화 시에만: 방산·우라늄·크립토·배터리·반도체 장비 등)
6. 원자재 · 금리 · 환율 (WTI·금·달러인덱스 + 10Y·2Y + HYG·TLT + USD/KRW·JPY)
7. 글로벌 증시 (ETF 프록시)
8. 국제정세 & 매크로 이벤트 (연준·인플레·지정학·실적 — 각 주장에 [[n]](url) 출처)
9. 오늘 한국장 관전 포인트
10. **전일 대비 변화** (직전 보고서와 diff)
11. ⚠️ 면책 조항
12. 데이터 품질 & 소스 *(말미)*
